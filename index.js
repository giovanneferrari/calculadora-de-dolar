...
async function converterMoeda() {
    ...
    try {
        ...
        let formattedDate = `${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}-${String(today.getFullYear())}`;
        ...
        while (data.value.length === 0) {
            ...
            formattedDate = `${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}-${String(today.getFullYear())}`;
        ...
        if (conversionType === "BRLtoUSD") {
            const valorEmDolar = (valor / taxaDeCambio).toLocaleString('pt-BR', { minimumFractionDigits: 4, maximumFractionDigits: 4 });
            resultado = `R$ ${valor} equivale a $ ${valorEmDolar}`;
        } else if (conversionType === "USDtoBRL") {
            const valorEmReais = (valor * taxaDeCambio).toLocaleString('pt-BR', { minimumFractionDigits: 4, maximumFractionDigits: 4 });
            resultado = `$ ${valor} equivale a R$ ${valorEmReais}`;
        } else if (conversionType === "BRLtoEUR") {
            const valorEmEuro = (valor / taxaDeCambio).toLocaleString('pt-BR', { minimumFractionDigits: 4, maximumFractionDigits: 4 });
        ...
        const taxaDeCambioFormatada = taxaDeCambio.toLocaleString('pt-BR', { minimumFractionDigits: 4, maximumFractionDigits: 4 });
...
// Função para obter o histórico dos últimos 7 dias para uma moeda específica
async function obterHistoricoMoeda(moeda) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        let formattedDate = `${String(dataAtual.getMonth() + 1).padStart(2, '0')}-${String(dataAtual.getDate()).padStart(2, '0')}-${String(dataAtual.getFullYear())}`;
        ...
        while (jsonData.value.length === 0) {
            ...
            formattedDate = `${String(dataAtual.getMonth() + 1).padStart(2, '0')}-${String(dataAtual.getDate()).padStart(2, '0')}-${String(dataAtual.getFullYear())}`;
...
// Função para exibir o histórico das cotações
function exibirHistorico(moeda) {
    obterHistoricoMoeda(moeda).then(historico => {
        const historicoElement = document.getElementById('historico');
        historicoElement.innerHTML = "<h3>Histórico das Cotações (Últimos 7 Dias)</h3>";

        if (moeda === "USD") {
            historicoElement.innerHTML += "<h4>Dólar (USD):</h4>";
            historico.forEach(entry => {
                historicoElement.innerHTML += `<p>${entry.data}: $ ${entry.cotacao.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}</p>`;
            });
        } else if (moeda === "EUR") {
            historicoElement.innerHTML += "<h4>Euro (EUR):</h4>";
            historico.forEach(entry => {
                historicoElement.innerHTML += `<p>${entry.data}: € ${entry.cotacao.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}</p>`;
            });
        }
