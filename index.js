            ...
            <table>
                ...
                <tbody>
                    ${historico.map(entry => `
                        <tr>
                            <td>${entry.data}</td>
                            <td>${moedaSimbolo} ${entry.cotacao.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>`;
...
async function converterMoeda() {
    ...
    try {
        ...
        let formattedDate = `${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate() + 1 ).padStart(2, '0')}-${String(today.getFullYear())}`;
